...
unsigned long lastFlightUpdate = 60 * 1000 * 2;
...
// Fetches detailed flight information from a secondary API (FlightAware).
...
// Fetches state vectors from OpenSky and finds the nearest valid flight
void getNearestFlightInfo() {
  ...
  String url = String(opensky_url) + "?lamin=" + String(LAT - 0.5) + "&lomin=" + String(LON - 0.5) + "&lamax=" + String(LAT + 0.5) + "&lomax=" + String(LON + 0.5);
...
void displayFlightInfo(String origin, String dest, String flight, String aircraft) {
  ...
  dma_display->drawFastHLine(dma_display->getCursorX() + 3, 16, 64 - (dma_display->getCursorX() + 3) - 4, dma_display->color565(0, 128, 255));
  ...
  // add distance from me and direction its traveling

  dma_display->flipDMABuffer();
}

void drawPlane(int px, int py) {

  uint16_t color = dma_display->color565(255, 255, 255);  // white plane
  ...
  for (int row = 0; row < 11; row++) {
    for (int col = 0; col < 12; col++) {
      if (plane[row][col] == '1') {
        ...
        // Bounds check so it doesn't crash when partially off-screen
...
void planeAnimationTask(void *parameter) {
  const TickType_t frameDelay = 40 / portTICK_PERIOD_MS;  // ~25 FPS

  while (1) {
    if (buttonClicked) {

      // Clear ONLY the bottom half
      for (int y = 20; y < 32; y++) {
        for (int x = 0; x < 64; x++) {
      ...
      // Draw the plane at its current X, Y=20
      drawPlane(planeX, 20);
